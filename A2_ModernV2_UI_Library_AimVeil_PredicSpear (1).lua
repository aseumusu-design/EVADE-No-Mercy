...
-- Aim Veil: predictive spear targeting, isolated from every other feature.
local A2VeilConfig = {
    Enabled = false,
    ShowFOV = true,
    FOV = 150,
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    if not A2VeilConfig.Enabled or not spearMode then A2VeilHideDrawings(); return end;
    ...
    elseif A2VeilDraw.FOVCircle then A2VeilDraw.FOVCircle.Visible = false end;
    local target = A2VeilClosestTarget();
    if target and target.Part and target.Part.Parent then
        A2VeilDraw.Highlight.Parent = target.Part.Parent;
        if A2VeilState.Charging then
            local startPart = character:FindFirstChild("Head") or character:FindFirstChild("HumanoidRootPart");
            if startPart then
                local velocity = A2VeilVelocity(target.Part, target.Player.Name);
                local _, predicted = A2VeilBallisticDirection(startPart.Position, target.Part.Position, velocity);
                A2VeilState.LastPredictedPos = predicted;
                A2VeilState.LastPredictedUntil = os.clock() + 0.12;
            end;
        end;
    else A2VeilDraw.Highlight.Parent = nil end;
    if A2VeilDraw.Tracer and A2VeilState.LastPredictedPos and os.clock() <= A2VeilState.LastPredictedUntil then
        local screen, onScreen = camera:WorldToViewportPoint(A2VeilState.LastPredictedPos);
        local center = Vector2.new(camera.ViewportSize.X * 0.5, camera.ViewportSize.Y * 0.5);
        if onScreen then
            ...
            A2VeilDraw.Tracer.Position = Vector2.new(center.X + dx * scale, center.Y + dy * scale);
        end;
        A2VeilDraw.Tracer.Visible = true;
    elseif A2VeilDraw.Tracer then A2VeilDraw.Tracer.Visible = false end;
end);
...
-- Floating menu icon toggles the window; it does not run any feature loop.
